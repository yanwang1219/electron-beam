...
[z_mesh, x_mesh] = meshgrid(z,x);                               % splite the space according to z,x
[Ez, Ex] = x_selector(z_mesh, x_mesh, plate_length, ...
    plate_left_z, plate_righ_z, d, n_q, q);
quiver(z_mesh, x_mesh, Ez, Ex);                                 % plot arrows with directional components
%% Run simulation
...
for i = 2:t_max + 1
    E_partical = [...
        interp2(z_mesh, x_mesh, Ez, ...
            trajectory(i-1, 1), trajectory(i-1, 2)), ...
        interp2(z_mesh, x_mesh, Ex, ...
            trajectory(i-1, 1), trajectory(i-1, 2))];               % get electriacl field strength applied on partical.
    trajectory(i,:) = ...
...
quiver(z_mesh, x_mesh, Ez, Ex);                                     % plot the electrical field
hold on;
xlim([0 space_size(1)]);
quiver(trajectory(1:i-1,1), ...                                     % plot the trajactory of electron
    trajectory(1:i-1,2), ...
